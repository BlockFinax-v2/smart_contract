...
describe("GovernanceFacet Comprehensive Tests", function () {
  ...
  describe("Voting System", function () {
    ...
    it("Should track vote counts", async function () {
      ...
      // Voting power is a percentage (0-1e18), with 2 equal financiers each has 0.5e18
      // Total voting power should be 1e18 (100%) when both vote
      const expectedTotal = ethers.parseEther("1"); // 0.5 + 0.5 = 1.0 (100%)
  ...
  */ // End of commented Financier Management section
  ...
  describe("Access Control", function () {
    ...
    it("Should restrict financier-only functions", async function () {
      ...
      // This would test financier-only functions if they exist
      // For now, we'll test that non-financier cannot perform certain actions
      expect(await governanceFacet.isFinancier(await user1.getAddress())).to.be.false;
    });
  });
